...
### Java server launcher behavior

`startJavaServer` automatically uses the launcher generated by each server type:

- Fabric: `fabric-server-launch.jar`
- Modern Forge / NeoForge: `server/run.sh`
- Vanilla / Paper / Spigot / Sponge / BungeeCord / legacy Forge: `server.jar`

For modern Forge and NeoForge, edit `server/user_jvm_args.txt` to change JVM memory/options. The generated `run.sh` and `libraries/.../unix_args.txt` files are intentionally left untouched.
